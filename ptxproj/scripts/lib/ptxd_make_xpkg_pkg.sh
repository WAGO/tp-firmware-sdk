...
ptxd_install_compression_format() {
    ...
    # FIXME: WAGO PTXdist-Update 2024.12.4
    # comp resolves to `, which is invalid, therefore we set it to empty
    comp=""
...
ptxd_install_generic() {
    ...
    case "${type}" in
	"directory")
        # When built witch build version 'binaries' 
        # (menuconfig -> wago specific -> build version -> binaries)
        # the package codesys3 is installed via install_archive.
        # In this package both sgid and gid are set
        # (ptxproj.2013.03.0/rules/wago-codesys3-000-rts.make:237).
        # With the commit 7dabb5c39d617a65c94a7d5610c4504f096ae319 from
        # Michael Olbrich (ptxdist upstream repo), the sgid will be dropped
        # in this specifc case. Therefore the change from said commit is
        # commented out.
...
ptxd_install_shared() {
    ...
    # Patched by WAGO:
    # We need to install plain .so files as well, so that CODESYS finds
