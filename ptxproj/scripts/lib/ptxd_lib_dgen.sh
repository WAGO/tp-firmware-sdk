...
ptxd_dgen_map_all() {
	...
	# We need a change in ptxd_lib_dgen.awk, which was taken from
	# PTXDIST_LIB directory. In order to use our version, the path
	# is changed.
	xargs gawk -f "$(realpath $(dirname $BASH_SOURCE))/ptxd_lib_dgen.awk"
    }
    check_pipe_status
}
