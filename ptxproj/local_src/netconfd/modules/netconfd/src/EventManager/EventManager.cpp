...
void EventManager::NotifyNetworkChanges(EventLayer event_layer) {
  NotifyNetworkChanges(event_layer, ::std::nullopt);
}
...
void EventManager::RegisterNetworkInformation(IPersistenceProvider &persistence_provider, IIPInformation &ip_information,
  ...
  if (dir_exists) {
    ...
    for (size_t i = 0; i < port_devs.size(); i++) {
      NotifyNetworkChanges(EventLayer::IP_CHANGE_FILES, Interface::CreateBridge("br"s.append(std::to_string(i))));
    }

    auto wwans = netdev_manager.GetNetDevs({DeviceType::Wwan});
    for (auto &wwan_dev : wwans) {
      NotifyNetworkChanges(EventLayer::IP_CHANGE_FILES, Interface{wwan_dev->GetInterface()});
    }
  }
}
...
::std::vector<char> EventManager::GetBridgeConfigAsJson() {
  ...
  if (persistence_provider_ != nullptr) {
    persistence_provider_->Read(bridge_config);
  }
