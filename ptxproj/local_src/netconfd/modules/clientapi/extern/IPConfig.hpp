...
class IPConfigs : public detail::ConfigBase<netconf::IPConfig> {
 public:
   ...
   */
  void RemoveIPConfig(const ::std::string& interface);
   ...
   *
   * @param interface_name Name of the interface
   * @return std::optional<IPConfig>
   */
  ::std::optional<netconf::IPConfig> GetIPConfig(const ::std::string& interface) const;
  #endif

  IPConfig& operator[](const ::std::string &interface);
 ...
 */
Status GetIPConfigs(const DeviceTypes& type, IPConfigs& configs);
 ...
 */
Status GetCurrentIPConfigs(const DeviceTypes& type, IPConfigs& configs);
 ...
 */
Status ValidateIpConfigs(const IPConfigs& candidate, const IPConfigs& current,
