...
TEST_F(libutil, SystemCall_emptyString) {
  auto ret = SystemCall(emptyPath.string());
  ASSERT_EQ(ERROR, ret);
}

TEST_F(libutil, SavePackageList_emptyString) {
  auto ret = SavePackageList(emptyPath.string());
  ASSERT_EQ(ERROR, ret);
}

TEST_F(libutil, CreateNewArchive_emptySrcList) {
  auto ret = CreateNewArchive(srcList, "test");
  ASSERT_EQ(ERROR, ret);
}

TEST_F(libutil, CreateNewArchive_emptyDst) {
  srcList.emplace_back("Test");
  auto ret = CreateNewArchive(srcList, emptyPath.string());
  ASSERT_EQ(ERROR, ret);
}

TEST_F(libutil, FolderToPathExist_emptyDst) {
  auto ret = FolderToPathExist(emptyPath.string());
  ASSERT_EQ(false, ret);
}
