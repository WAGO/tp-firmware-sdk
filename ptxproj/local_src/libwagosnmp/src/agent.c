...
netsnmp_handler_registration *__attribute__((weak))
netsnmp_create_handler_registration(const char *name, Netsnmp_Node_Handler *handler_access_method, const oid *reg_oid,
...
netsnmp_set_request_error(netsnmp_agent_request_info *reqinfo, netsnmp_request_info *request, int error_value) {
  (void)reqinfo;
  (void)request;
  (void)error_value;
  return 1;
...
int __attribute__((weak)) netsnmp_send_traps(int trap, int specific, const oid *enterprise, int enterprise_length,
...
static void _TrapSend(tWagoSnmpMsg *trap) {
  ...
  if (trap->variable.type != ASN_NULL) {
    snmp_varlist_add_variable(&notification_vars, trap->variable.sOID, trap->variable.sOID_length, trap->variable.type,
                              (u_char *)trap->variable.buf, trap->variable.len);
  }
  netsnmp_send_traps(trap->trap_type, trap->specific_type, trap->enterprise, trap->enterprise_length, notification_vars,
                     NULL, 0);
...
static void _RegisterOIDHandler(oid *anOID, size_t anOID_length) {
  ...
  new_handler =
      netsnmp_create_handler_registration(WAGOSNMP_AGENT_NAME, _OidHandler, anOID, anOID_length, HANDLER_CAN_RWRITE);
  if (new_handler) {
    // register handler
...
static void *_ServerMain(void) {
  while (1) {
    ...
    if (0 < poll(&fdrec, 1, -1)) {
      ...
      if (0 < mq_receive(trap_queue, (char *)&stMessage, sizeof(tWagoSnmpMsg), NULL)) {
