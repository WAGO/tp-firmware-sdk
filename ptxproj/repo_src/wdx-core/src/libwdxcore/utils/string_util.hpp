...
///  \file
//------------------------------------------------------------------------------
#ifndef SRC_LIBWDXCORE_UTILS_STRING_UTIL_HPP_
#define SRC_LIBWDXCORE_UTILS_STRING_UTIL_HPP_
...
inline std::string replace(std::string const &src, std::string const &token, std::string const &replacement)
{
    // https://stackoverflow.com/a/29207548
    return std::regex_replace(src, std::regex(token), replacement);    
}

static std::unique_ptr<std::default_random_engine> random_engine;

// https://stackoverflow.com/a/12468109
inline std::string random_alpha(size_t const &length)
{
    if(!random_engine)
    {
        // obtain a seed from the system clock:
        unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
        random_engine = std::make_unique<std::default_random_engine>(seed);
    }
    ...
    auto randchar = []() -> char
    {
        ...
        return charset[(*random_engine)() % max_index];
    };
...
#endif // SRC_LIBWDXCORE_UTILS_STRING_UTIL_HPP_
