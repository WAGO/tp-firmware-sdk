...
///  \file
///
///  \brief    Verify document response for WAGO auth service.
///
///  \author   PEn: WAGO GmbH & Co. KG
//------------------------------------------------------------------------------
#ifndef SRC_LIBAUTHSERV_VERIFY_DOCUMENT_HPP_
#define SRC_LIBAUTHSERV_VERIFY_DOCUMENT_HPP_
...
class verify_document : public http::response_i
{
private:
...
public:
    // Creates a minimal document for an invalid token
    verify_document();

    // Creates a document for a valid token
    verify_document(std::string const &username,
                    std::string const &scopes,
                    uint32_t           expire);
    ~verify_document() noexcept override;
...
#endif // SRC_LIBAUTHSERV_VERIFY_DOCUMENT_HPP_
//---- End of source file ------------------------------------------------------
