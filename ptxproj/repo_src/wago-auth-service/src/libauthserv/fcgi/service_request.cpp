...
using authserv::http::get_method_from_name;

constexpr size_t service_request::max_content_size;
...
bool service_request::accept()
{
    ...
    if(request_state_m == request_states::initial)
    {
        ...
        uri_m              = http::uri(raw_uri);
        query_parameters_m = uri_m.parse_query();
        if(request_accepted && is_content_type_matching("application", "x-www-form-urlencoded"))
        {
            auto content_length_raw = get_content_length();
            size_t processed = 0;
            if(stoul(content_length_raw, &processed) > max_content_size)
            {
                request_accepted = false;
                wc_log(log_level_t::warning, "Got request with too large content size, reject request.");
                respond(http::head_response(http::http_status_code::payload_too_large));
                WC_FAIL("Payload / content size too large"); // Should never hit, webserver should filter to large requests
            }
            else if(processed != content_length_raw.length())
            {
                request_accepted = false;
                wc_log(log_level_t::warning, "Got request with invalid content size, reject request.");
                respond(http::head_response(http::http_status_code::bad_request));
                WC_FAIL("Payload / content size too large"); // Should never hit, webserver or FCGI should filter invalid requests
            }
            else
            {
                // we can re-use the query-string parsing function as 
                // form data parameters are encoded as 
                // application/x-www-form-urlencoded.
                form_parameters_m = http::uri::parse_query(get_content());
            }
        }
        wc_log_format(log_level_t::debug, "Received request from '%s' with method '%s', path '%s' and query '%s': %s",
                      remote_host_m.c_str(), http::get_method_name(method_m),
...
http::uri service_request::get_request_uri() const
{
    return uri_m;
}
...
bool service_request::has_form_parameter(std::string const & name) const
{
    return form_parameters_m.count(name) > 0;
}

std::string service_request::get_form_parameter(std::string const & name, bool decode) const
{
    return decode ? decode_url(form_parameters_m.at(name)) : form_parameters_m.at(name);
}

bool service_request::has_http_header(std::string const &header_name) const
{
    std::string upper_header_name(header_name);
    std::transform(upper_header_name.begin(), upper_header_name.end(),upper_header_name.begin(), ::toupper);
...
void service_request::respond(response_i const &response) noexcept
{
    ...
    authserv::http::http_status_code const  http_status_code      = response.get_status_code();
    char                             const *http_status_code_text = authserv::http::get_http_status_code_text(http_status_code);
    ...
    wc_log_format(log_level_t::debug, "Respond request from '%s' with method '%s', path '%s' and query '%s': HTTP status %u (%s)",
                  remote_host_m.c_str(), wago::authserv::http::get_method_name(method_m),
    ...
    try
    {
        ...
        if(!response_header_m.empty())
        {
            ...
            for(std::pair<std::string, std::string> header_pair : response_header_m)
            {
        ...
        {
            write_response_data("Content-Type:   " + response.get_content_type()   + "\r\n", request_m->out);
        }
        if(!response.get_content_length().empty())
        {
            write_response_data("Content-Length: " + response.get_content_length() + "\r\n", request_m->out);
        }   
        write_response_data("\r\n", request_m->out);
        // can send body now
...
void debug_log_request(FCGX_Request const *request)
{
    ...
    if(wc_get_log_level() == log_level_t::debug)
    {
        ...
        for(int i = 0; p[i] != nullptr; ++i)
        {
            ...
            params += ") ";
            params += p[i];
            wc_log(log_level_t::debug, params);
            params = "";
        }
