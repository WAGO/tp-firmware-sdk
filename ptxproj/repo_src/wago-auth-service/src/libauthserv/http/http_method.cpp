...
http_method get_method_from_name(char const * const method_name)
{
    ...
    for(unsigned i = 0; i <= static_cast<unsigned>(http_method::max_num); ++i)
    {
        ...
        {
            return static_cast<http_method>(i); //parasoft-suppress CERT_CPP-INT50-a-3 "Enum values of status codes are explicitly designed to be searched. Values are defined without gap and with an explicit end marker."
        }
    }
    throw wago::authserv::exception(std::string("Unknown HTTP method: ") + method_name);
}
