...
{
    return wdx::linuxos::tolower_copy(std::regex_replace(path.device_path + "-" + path.parameter_path, std::regex("/"), "-"));
}
...
{
    auto fragments = wdx::linuxos::split_string(s, '-');
    if(fragments.size() != 2)
    {
    ...
    int coll = 0;
    int slot = 0;
    if(!wdx::linuxos::convert_to_int(fragments[0], coll))
    {
        throw std::runtime_error("Malformed string representing a device_id.");
    }
    if(!wdx::linuxos::convert_to_int(fragments[1], slot))
    {
        throw std::runtime_error("Malformed string representing a device_id.");
    }
    if(slot < 0 || coll < 0)
...
wago::wdx::status_codes check_device_id_ranges(std::string                       const &collectionStr,
                                               ...
                                               wago::wdx::slot_index_t                 &slot)
{
    ...
    int c = 0;
    if(!wdx::linuxos::convert_to_int(collectionStr, c))
    {
        return wago::wdx::status_codes::invalid_device_collection;
    }
    ...
    coll = c;
    int s = 0;
    if(!wdx::linuxos::convert_to_int(slotStr, s))
    {
...
parameter_instance_id from_string<parameter_instance_id>(std::string const &s)
{
    ...
    auto status = check_device_id_ranges(fragments[0], fragments[1], coll, slot);
    if(has_error(status))
    {
...
wago::wdx::parameter_instance_path from_string<wago::wdx::parameter_instance_path>(std::string const &s)
{
    ...
    wago::wdx::device_collection_id_t coll = 0;
    wago::wdx::slot_index_t slot = 0;
    auto status = check_device_id_ranges(m.str(2), m.str(3), coll, slot);
    if(has_error(status))
    {
        throw std::runtime_error("String representing a parameter_instance_path has invalid parts.");
    }
    auto device = std::regex_replace(m.str(1), std::regex("-"), "/");
    auto parameter = std::regex_replace(m.str(4), std::regex("-"), "/");
...
wago::wdx::parameter_id_t from_string<wago::wdx::parameter_id_t>(std::string const &s)
{
    ...
    uint32_t def = 0;
    if(!wdx::linuxos::convert_to_uint32(s, def))
    {
...
{
    int inst = 0;
    if(!wdx::linuxos::convert_to_int(s, inst))
    {
...
{
    int coll = 0;
    if(!wdx::linuxos::convert_to_int(s, coll))
    {
...
wago::wdx::monitoring_list_id_t from_string<wago::wdx::monitoring_list_id_t>(std::string const &s)
{
    ...
    uint64_t def = 0;
    if(!wdx::linuxos::convert_to_uint64(s, def))
    {
...
wago::wdx::status_codes from_string<wago::wdx::status_codes>(std::string const &s)
{
    ...
    return static_cast<wago::wdx::status_codes>(val); // parasoft-suppress CERT_CPP-INT50-a-3 "Value came from status_codes enum before"
}
