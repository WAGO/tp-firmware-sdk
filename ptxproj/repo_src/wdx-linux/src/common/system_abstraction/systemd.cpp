...
{
    WC_DISBALE_CLASS_COPY_ASSIGN_AND_MOVE_ASSIGN(systemd_impl)
...
int systemd_impl::find_systemd_socket(std::string const &socket_name) const
{
    ...
    wc_log(log_level_t::info, ("Got " + std::to_string(fd_count) + " file descriptor(s) from systemd.").c_str());
    ...
    for(size_t i = 0; (i < fd_count) && (fd_names[i] != nullptr); ++i)
    {
        if(socket_name == fd_names[i])
        {
            ...
            wc_log(log_level_t::info, ("Found socket file descriptor for \"" + socket_name + "\" as FD" + std::to_string(fd) + ".").c_str());
        }
...
void systemd_impl::notify_ready() const
{
    ...
    WC_DEBUG_LOG(("Result of systemd notify for READY: " + std::to_string(send_result)).c_str());
...
void systemd_impl::notify_stopping() const
{
    ...
    WC_DEBUG_LOG(("Result of systemd notify for STOPPING: " + std::to_string(send_result)).c_str());
    // It is recommended to further ignore the return value of notify
    // See: https://www.freedesktop.org/software/systemd/man/latest/sd_notify.html#
}
