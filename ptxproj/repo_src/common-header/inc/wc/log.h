...
#define WC_LOG_FORMAT_BUFFER_SIZE                                         (256u)
...
#define WC_LOG_BUFFER_SIZE                                               (4096u)
...
/// \see <a href="https://betterstack.com/community/guides/logging/log-levels-explained/">Log Levels Explained and How to Use Them</a>
typedef enum log_level
{
  ...
  /// Notice
  /// The Notice level is typically used to output information that indicates normal operation normal but with
  /// significant conditions that may require monitoring.
  notice = 4,   //!< notice
...
extern "C"
{
  ...
  void wc_log_output(log_level_t  const log_level,
                     char const * const message) NO_THROW;
  ...
  //------------------------------------------------------------------------------
  log_level_t wc_get_log_level(void) NO_THROW;
  ...
  static inline void wc_log_format(log_level_t  const log_level,
                                   ...
                                   ...) NO_THROW GNUC_ATTRIBUTE(format (gnu_printf, 2, 3));  //lint !e960 !e1916 Variable number of arguments needed
  ...
  static inline void wc_vlog_format(log_level_t  const log_level,
                                    char const * const format,
                                    va_list *    const args) NO_THROW GNUC_ATTRIBUTE(format (gnu_printf, 2, 0));
  ...
  static inline void wc_log_format_advanced(char const * const file,
                                            ...
                                            char const * const format,
                                            ...) NO_THROW GNUC_ATTRIBUTE(format (gnu_printf, 5, 6));  //lint !e960 !e1916 Variable number of arguments needed 
  ...
  static inline void wc_vlog_format_advanced(char const * const file,
                                             ...
                                             log_level_t  const log_level, 
                                             char const * const format,
                                             va_list *    const args) NO_THROW GNUC_ATTRIBUTE(format (gnu_printf, 5, 0));
  ...
  /// Log function (C variant) to log static messages with source
  /// (file, function, line) information.
  /// Source information is only computed when currently used application log level
  /// is "debug", determined by function \link wc_get_log_level \endlink.
  ///
  /// \param file
  ...
  /// Simple log function (C variant) to log static messages.
  ///
  /// \param log_level
  ///   Log level for message, used to forward or discard message dependent on
  ///   currently active application log level.
                                             ...
                                             va_list * const    args) NO_THROW
  {
    if ( (log_level > off) && (wc_get_log_level() >= log_level) ) 
    {
      ...
      if ( (wc_get_log_level() == debug) && (file != NULL) ) {
        ...
        {
          file_shortend += (file_length - WC_LOG_MAX_FILE_LENGTH);
        }
        ...
        {
          function_shortend += (function_length - WC_LOG_MAX_FUNCTION_LENGTH);
        }
        ...
        strncat(buffer, format, (WC_LOG_FORMAT_BUFFER_SIZE - 1u));
        buffer[WC_LOG_FORMAT_BUFFER_SIZE - 1u] = '\0';
      ...
      out_buffer[WC_LOG_BUFFER_SIZE - 1u] = '\0';
      WC_ASSERT(print_result >= 0);

      if(print_result > 0)
      {
        if(print_result > (signed)WC_LOG_BUFFER_SIZE)
        {
  ...
  ///   Maximum log message size in bytes.
  //------------------------------------------------------------------------------
  static inline constexpr size_t wc_get_max_log_message_size() NO_THROW
  {
    return WC_LOG_BUFFER_SIZE;
  }
  ...
  ///   Maximum log format size in bytes.
  //------------------------------------------------------------------------------
  static inline constexpr size_t wc_get_max_log_format_size() NO_THROW
  {
    return WC_LOG_FORMAT_BUFFER_SIZE;
  }

  /// Log function (C++ variant) to log a static message with source
  /// (file, function, line) information.
  /// Source information is only computed when currently used application log level
  /// is "debug", determined by function \link wc_get_log_level \endlink.
  ///
  ...
  /// Log function (C++ variant) to log a message string with source
  /// (file, function, line) information.
  /// Source information is only computed when currently used application log level
  /// is "debug", determined by function \link wc_get_log_level \endlink.
  ///
  ...
  ///   currently active application log level.
  /// \param message
  ///   Log message as std::string to compute.
  ///
  /// \see wc_get_log_level
  /// \see wc_log
  /// \see wc_log_format_advanced
  ...
  static inline void wc_log_advanced(char const * const  file,
                                     ...
                                     size_t       const  line,
                                     log_level_t  const  log_level, 
                                     std::string  const &message) NO_THROW
  {
    wc_log_advanced(file, function, line, log_level, message.c_str());
  }

  /// Simple log function (C++ variant) to log a message string.
  ///
  /// \param log_level
  ///   Log level for message, used to forward or discard message dependent on
  ///   currently active application log level.
  ...
  /// Simple log function (C++ variant) to log a static message.
  ///
  /// \param log_level
  ///   Log level for message, used to forward or discard message dependent on
  ///   currently active application log level.
  ...
  static inline void wc_log(log_level_t const  log_level,
                            std::string const &message) NO_THROW
  {
    wc_log(log_level, message.c_str());
  }
